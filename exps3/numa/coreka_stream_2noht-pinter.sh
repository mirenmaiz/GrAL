...
for i in `seq 0 9`; do 
	...
	../../rapl_logger.sh ../../allocator ${core_list_s0[$i]} ../../stream.bin ${core_list_s0[$j]} ../../stream.bin > coreka_stream-${core_list_s0[$i]}-${core_list_s0[$j]}-s$s-pinter.log
	sleep 2
done

for i in `seq 0 9`; do 
	let j=($i+1)%10
	../../rapl_logger.sh ../../allocator ${core_list_s1[$i]} ../../stream.bin ${core_list_s1[$j]} ../../stream.bin > coreka_stream-${core_list_s1[$i]}-${core_list_s1[$j]}-s$s-pinter.log
	sleep 2
done
